<?php 
	...
	if (isset($_POST['insert-bus'])) {
		...
		if ($admin=="" || $category=="" || $source=="" || $destination=="" || $title=="" || $intermediate=="" || $date=="" || $via_time=="" || $bus_detail=="" || $max_seats=="") {
			...
			$query = "INSERT INTO posts(post_category_id, post_title, post_author, post_date, post_image, post_content, post_source, post_destination, post_via, post_via_time, max_seats, available_seats) 
			VALUES({$category}, '{$title}', '{$admin}', '{$date}', '{$image}', '{$bus_detail}', '{$source}', '{$destination}', '{$intermediate}', '{$via_time}', $max_seats, $max_seats)";
...
<div style="background-color:#CEE9B6;width:1350px;width:1050px; margin:10px; padding:10px;border:5px solid black; border-radius:10px;">

<form action="" method="post" enctype="multipart/form-data">
<table style="border:5px solid black; border-radius:10px; width:2px;">
